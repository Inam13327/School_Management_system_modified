...
const ReportCardPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const studentRes = await fetch(`http://192.168.18.139:8000/api/students/${studentId}/`);
      if (studentRes.ok) {
        setStudent(await studentRes.json());
      }
      // Fetch subjects for class
      const subRes = await fetch(`http://192.168.18.139:8000/api/subjects/?class_fk=${classId}`);
      let subjectList = [];
      if (subRes.ok) {
      ...
      const marksRes = await fetch(`http://192.168.18.139:8000/api/marks/?class_fk=${classId}&student=${studentId}`);
      let marksList = [];
      if (marksRes.ok) {
      ...
      const attRes = await fetch(`http://192.168.18.139:8000/api/attendances/?student=${studentId}&student__class_fk=${classId}`);
      let attList = [];
      if (attRes.ok) {
