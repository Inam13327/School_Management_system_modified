...
const Students = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        ...
        try {
          const testResponse = await fetch('http://192.168.18.139:8000/api/students/', {
            method: 'GET',
            headers: {
          ...
          throw new Error('Cannot connect to server. Please make sure the Django server is running on http://192.168.18.139:8000');
        }
          ...
          data.map(s => {
            ...
            if (s.pic) {
              if (s.pic.startsWith('http')) {
                ...
                picUrl = `http://192.168.18.139:8000${s.pic}`;
              } else {
                picUrl = `http://192.168.18.139:8000/media/student_pics/${s.pic}`;
              }
            }
            // Debug log
  ...
  const handleAddStudent = async (e) => {
    ...
    try {
      const response = await fetch('http://192.168.18.139:8000/api/students/', {
        method: 'POST',
        body: formData,
      });
      if (!response.ok) {
      ...
      if (selected.classIdx !== null && selected.gender) {
        const classId = selected.classIdx + 1;
        const res = await fetch(`http://192.168.18.139:8000/api/students/?class_admitted=Class ${classId}&gender=${selected.gender}`);
  ...
  const handleDeleteStudent = async (studentId) => {
    ...
    setError(null);
    try {
      const response = await fetch(`http://192.168.18.139:8000/api/students/${studentId}/`, {
        method: 'DELETE',
      });
  ...
  const handleUpdateStudent = async (e) => {
    ...
    try {
      const response = await fetch(`http://192.168.18.139:8000/api/students/${editId}/`, {
        method: 'PUT',
        body: formData,
      });
      if (!response.ok) {
